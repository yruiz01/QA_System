...
public function cantidadalumnos(){
    $sql = "SELECT COUNT(*) AS total_alumnos FROM alumn";
    return ejecutarConsulta($sql);
}


public function cantidadalumnos_porgrupo($idgrupo) {
    $sql = "SELECT a.id as idalumno, a.name, a.lastname, a.image 
            FROM alumn a 
            INNER JOIN alumn_team alt ON a.id = alt.alumn_id 
            WHERE alt.team_id = '$idgrupo'";
    
    return ejecutarConsulta($sql); 
}

public function cantidadg($idgrupo) {
...
public function cantidadgrupos() {
    $sql = "SELECT idgrupo, nombre FROM team"; 
    return ejecutarConsulta($sql);
}
